...
try:
	times = float(input("Введите время: "))
	freq = float(input("Введите частоту синусоидального сигнала: "))
	samplingFrequency = int(input("Введите частоту семплирования: "))
	samplingPeriod = float(1 / samplingFrequency)
	if freq <= 0 or times <= 0 or samplingFrequency <= 0:
		print("Вы ввели некорректное число. Производится выход из программы")
		exit()
	ndarray = makesin(times, freq, samplingFrequency)
	for i in range(0, int(times * samplingFrequency + 0.5), 1):
		num2dac(ndarray[i])
